...
import { getLoopringInfo } from '@/api/user'
import { type } from 'os'
...
export default {
  ...
  data() {
    return {
      ...
      infoData: {},
      dateList: [],
      lineTitle: "Total Users",
      dateTitle: "Loopring"
    }
  },
  mounted() {
    this.reuqestLoopringInfo()
  },
  filters: {
  ...
  methods: {
    reuqestLoopringInfo() {
      getLoopringInfo().then(response => {
        console.log(response.data)
        this.infoData = response.data
        this.groupData = response.data.list.slice(-1)[0]
        this.dateTitle = "Loopring " + this.formatDate(this.groupData.createTime, true)
